...
tabla_label = tk.Label(root, text="Tabla:")
...
def download_data():
    ...
    query = "SELECT * FROM %s WHERE FECHAHORAOCURRENCIA BETWEEN '%s' AND '%s' ORDER BY ID DESC;" % (tabla, start_date_string, end_date_string)
        ...
        if(tabla == "OP_Registro"):
            ...
            if file_path:
                with open(file_path, mode='w', newline='') as file:
                    ...
                    writer.writerow(["id","idManatee","estadoEnvio","estadoENvioManatee", "fechahoraenvio", "fechahoraocurrencia","fechaprimerintento","trama"]) # replace "col2", "col3", ... with actual column names
                    for row in result:
                        writer.writerow(row)
...
def enable_download_button(event):
	try:
		...
		pass
start_date_entry.bind("<<DateEntrySelected>>", enable_download_button)
end_date_entry.bind("<<DateEntrySelected>>", enable_download_button)

download_button = tk.Button(root, text="Descargar", font=("Helvetica", 16), command=download_data, state="disabled")
